...
| <a id="rust_toolchain-target_json"></a>target_json |  Override the <code>target_triple</code> with a custom target specification. For more details see: https://doc.rust-lang.org/rustc/targets/custom.html If <code>target_triple</code> is absent, it falls back to the specification's <code>llvm-target</code> value. The custom target specification file is named with the first available of: (1) the provided <code>target_triple</code> attribute, (2) the specification's <code>llvm-target</code> value, (3) this target's <code>name</code>.   | String | optional | <code>""</code> |
